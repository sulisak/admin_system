...
if (isset($_SESSION['SESS_MEMBER_POS'])) {
	if ($_SESSION['SESS_MEMBER_POS'] == "Administration") {
		...
		class AccidentReport extends Controller
		{
			/**
			 * PAGE: index
			...
			public function index()
			{
				...
				if ($Sid == "") {
					...
					if ($LogUser == 1) {
						...
						require 'application/views/accident/index.php';
					} else {
						session_unset();
						session_destroy();
			...
			public function ListData()
			{
				...
				if ($Sid == "") {
					...
					if ($LogUser == 1) {
						...
						$AccidentModel = $this->loadModel('AccidentModel');
						$list1 = $AccidentModel->LoadAccidentget($start . " 01:00:00", $end . " 23:59:59");

						//print_r($list1);
						require 'application/views/_templates/header.php';
						require 'application/views/_templates/nav.php';
						require 'application/views/accident/listdata.php';
					} else {
						session_unset();
						session_destroy();
			...
			public function LoadAccident()
			{
				...
				if ($Sid == "") {
					...
					if ($LogUser == 1) {
						...
						$AccidentModel = $this->loadModel('AccidentModel');
						$list1 = $AccidentModel->LoadAccident();


						require 'application/views/accident/accident.php';
					} else {
						session_unset();
						session_destroy();
			...
			public function deleteAccident()
			{
				...
				if ($Sid == "") {
					...
					if ($LogUser == 1) {
						...
						$AccidentModel = $this->loadModel('AccidentModel');
						$AccidentModel->deleteAccident($id);
					} else {
						session_unset();
						session_destroy();
			...
			public function AddAccident()
			{
				...
				if ($Sid == "") {
					...
					if ($LogUser == 1) {
						...
						$AccidentModel = $this->loadModel('AccidentModel');
						$AccidentModel->AddAccident($vehicle, $name, $remarks, $date);
